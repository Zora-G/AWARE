        ...
        for i in 0..NLEN {
            ...
            unsafe {
                core::ptr::write_volatile(
                    &mut self.w[i],
                    t.wrapping_mul(c0).wrapping_add(s.wrapping_mul(c1)),
                )
            }
            ...
            unsafe {
                core::ptr::write_volatile(
                    &mut g.w[i],
                    s.wrapping_mul(c0).wrapping_add(t.wrapping_mul(c1)),
                )
            }
    ...
    pub fn cmove(&mut self, g: &BIG, b: isize) -> Chunk {
        ...
        for i in 0..NLEN {
            ...
            unsafe {
                core::ptr::write_volatile(
                    &mut self.w[i],
                    t.wrapping_mul(c0).wrapping_add(s.wrapping_mul(c1)), /* c0*t+c1*s */
                )
            }
